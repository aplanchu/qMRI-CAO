...
class ConcreteAutoencoder(pl.LightningModule):
    def __init__(
        ...
        prct_norm: float = 0.975,
        profiler=None,
    ) -> None:
        """Trains a concrete autoencoder. Implemented according to [_Concrete Autoencoders for Differentiable Feature Selection and Reconstruction_](https://arxiv.org/abs/1901.09346). Trains a network to estimate the parameters of a specified model (ZEBRA as default) and predict the signal following the model optimising the loss on the parameters instead of the signal

        Args:
            gt_path: path to the file (nifti) with ground truth quantitative parameters
            mridata_path: path to the file (txt) with the acquisition parameters
            ...
            prct_norm (float, optional): value between 0 and 1 that indicates the limit to normalise the diverse parameters
        """
        ...
        self.num_vols = input_output_size
        self.num_pars = latent_size2
        ...
        theta = torch.tensor(gt[:,:,:,0])
        theta = theta[theta!=0]
        min_theta = torch.min(theta)
        max_theta = torch.nanquantile(theta,prct_norm)

        phi = torch.tensor(gt[:,:,:,1])
        phi = phi[phi!=0]
        min_phi = torch.min(phi)
        max_phi = torch.nanquantile(phi,prct_norm)
        
        dpar = torch.tensor(gt[:,:,:,2])
        dpar = dpar[dpar!=0]
        min_dpar = torch.min(dpar)
        max_dpar = torch.nanquantile(dpar,prct_norm)
        
        kperp = torch.tensor(gt[:,:,:,3])
        kperp = kperp[kperp!=0]
        min_kperp = torch.min(kperp)
        max_kperp = torch.nanquantile(kperp,prct_norm)
        
        t2 = torch.tensor(gt[:,:,:,4])
        t2 = t2[t2!=0]
        min_t2 = torch.min(t2)
        max_t2 = torch.nanquantile(t2,prct_norm)
        
        t1 = torch.tensor(gt[:,:,:,5])
        t1 = t1[t1!=0]
        min_t1 = torch.min(t1)
        max_t1 = torch.nanquantile(t1,prct_norm)
        
        s0 = torch.tensor(gt[:,:,:,6])
        s0 = s0[s0!=0]
        min_s0 = torch.min(s0)
        max_s0 = torch.nanquantile(s0,prct_norm)
        
        self.param_min = torch.tensor([min_theta, min_phi, min_dpar, min_kperp, min_t2, min_t1, min_s0])
        self.param_max = torch.tensor([max_theta, max_phi, max_dpar, max_kperp, max_t2, max_t1, max_s0])
    ...
    def add_model_specific_args(parent_parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument(
            ...
            type=float,
            metavar="N",
            help="maximum temperature of the concrete layer (default: 10.0)",
        )
        parser.add_argument(
            "--min_temp",
            type=float,
            default=0.1,
            metavar="N",
            help="minimum temperature of the concrete layer (default: 0.1)",
        )
        ...
        parser.add_argument(
            ...
            type=file_path,
            metavar="PATH",
            help="Nifti with the synthetic ground truth maps",
        )
        parser.add_argument(
            "--mridata_path",
            default=None,
        ...
        parser.add_argument(
            ...
            type=float,
            metavar="N",
            help="Percentile selected to normalise the parameters",
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        encoded = self.encoder(x[:,:self.num_vols]);
        decoded, decoded2 = self.qmrizebra(encoded) # Actually, this decoded should be "encoded", but it has the name "decoded" to be returned together with the predicted signal and respect the original notation
    ...
    def _shared_eval(
        ...
        """
        #signals = batch[:,:self.num_vols].clone()
        maps = batch[:,-self.num_pars:].clone()
        ...
        for i in range(maps.shape[-1]):
            maps[:,i] = (maps[:,i]-self.param_min[i])/(self.param_max[i]-self.param_min[i]) # to avoid bias caused by a parameter with higher values
            decoded[:,i] = (decoded[:,i]-self.param_min[i])/(self.param_max[i]-self.param_min[i])
        
        loss = F.mse_loss(decoded, maps)
