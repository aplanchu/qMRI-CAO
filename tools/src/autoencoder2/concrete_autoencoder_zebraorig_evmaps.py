...
class ConcreteAutoencoder(pl.LightningModule):
    def __init__(
        ...
        """Trains a network to estimate the parameters of a specified model (ZEBRA as default) and predict the signal following the model optimising the loss on the parameters instead of the signal
        ...
        #self.num_vols = input_output_size # must be indicated manually for this particular case
    ...
    def _shared_eval(
        ...
        """
        #signals = batch[:,:self.num_vols]
        maps = batch[:,-self.num_pars:]
        decoded, decoded2 = self.forward(batch)
        if self.param_min.type != batch.type:
            self.param_min = self.param_min.type_as(batch)
            self.param_max = self.param_max.type_as(batch)
