...
from concrete_autoencoder_zebra_all_series2_evmaps import ConcreteAutoencoder
...
def trainer(args: Namespace) -> None:
    ...
    # Save the indexes of the selected measurements
    np.savetxt(args.path_save_ind,np.array(model.get_indices(), dtype=int), fmt='%d')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="path to save the selected indexes/measurements",
    )
    parser.add_argument(
        "--folder_hyperparams",
        default=None,
