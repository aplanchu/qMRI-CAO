...
class MRIDataModule(pl.LightningDataModule):
    def __init__(
        ...
        #self.num_workers = VALUE # MODIFY ACCORDING TO PERSONAL OPTIONS
    ...
    def train_dataloader(self) -> DataLoader:
        if self.in_memory:
            return DataLoader(
                self.train_set, batch_size=self.batch_size, shuffle=True, generator=torch.Generator(device='cuda'), 
            )
        else:
            return DataLoader(
                self.train_set,
    ...
    def val_dataloader(self) -> DataLoader:
        if self.in_memory:
            return DataLoader(self.val_set, batch_size=self.batch_size, shuffle=False, generator=torch.Generator(device='cuda'), drop_last=True,)
        else:
            return DataLoader(
                self.val_set,
                batch_size=self.batch_size,
